...
to gpuser-setup-syntax
  ...
  (list "maze-forward" TYPE_COMMAND 30)
  ...
  ;; This list contains reporters and operators which report numeric values
  set gp-syntaxlist-numeric [ ]
  
  ;; Boolean ingredients for codeturtle's random code generation
  ;; This list contains reporters and operators which report boolean values
  set gp-syntaxlist-boolean [ ]
end
...
;;    to run before or after a generation goes by, you can put it here.
to go
  ...
  [
    ifelse (gp-raw-fitness = 0)
      [ set shape "face happy" ]
      [ set shape "face neutral" ]
  ]
  ;; if we found a perfect turtle, then stop.
  if (gp-best-fitness-this-gen = 0)   
    [ stop ]
end
