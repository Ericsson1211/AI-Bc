;Asynchronous weak-commitment search with nogood processor centralized for the graph-coloring problem
...
nodes-own [the-links neighbors-list the-neighbors message-queue priority nogood_list nogood_sent_list
             colorn current-view messages-received_ok messages-received_nogood nr_constraintc  messages-received 
             nr nrn  priorities_list ]
...
globals [x-max y-max diam friction tot-edges filename time stoptick init-power-edges domain-color-list no-more-messages done tmp nr_cicluri nn
 priorities_list_receive nogood_list_receive nr_nogood_store] 
...
to setup-awcs
  ...
  set done false
  set nr_cicluri 0
  set nogood_list_receive []
  set priorities_list_receive []
  ask nodes [
    initialize
    set nrn 0
...
to handle-nogood-message [content_colors content_priorities ]
 let i 0
    ...
    set priorities_list (lput content_priorities priorities_list)
    if not member? content_colors nogood_list_receive
    [ 
    set nogood_list_receive (lput content_colors nogood_list_receive)
    set priorities_list_receive (lput content_priorities priorities_list_receive)
    set nr_nogood_store nr_nogood_store + 1
    ]
    ]
    set nrn nrn + 1
    set i 0
...
to-report findValue
  ...
  let result 0
   ...
   [   
       if (temp-val != colorn)
       [
       if  not (inconsistent-view temp-val -1)  and not inconsistent-nogood-processor temp-val
       [
           repeat (number-of-nodes)
           [ 
...
;reports a list with the priorities from current view 
to-report nogood-priorities-only [nogoods ]
    let i 0
  let result 0
    ...
    repeat (number-of-nodes)
    [
        ifelse (i = who) and (item i nogoods != -1)
            [set result (lput -1 result)]
            [set result (lput (last (item i current-view)) result)]
        set i i + 1
    ]
    report result
...
to-report inconsistent-nogood-processor [color-value]
   let i 0
  let result 0
  let temp-nogood 0
  ...
  let pos 0
   ...
   set result true
   ifelse not empty? nogood_list_receive
   [foreach nogood_list_receive
   [
     set pos position ? nogood_list_receive
     set temp-nogood ?
     set temp-priorities item pos priorities_list_receive
     set i 0    
     while [ i < number-of-nodes and result != false]
     [ 
       if (i != who and (greater-priority who priority i (last (item i current-view)) ))
       [
           ;set nr_comp  nr_comp + 1
           if (first (item i current-view) != item i temp-nogood ) and (first (item i current-view) != -1 ) and (item i temp-nogood != -1)
           [
               set result false
               report result
           ]
      ...
      ;set nr_comp  nr_comp + 1 
      if (item who temp-nogood != -1) and (item who temp-nogood != color-value)
       [set result false
        report result]
       set i i + 1
   ]
...
to backtrack 
  ...
  let newValue 0
    ...
    if not (member? nogoods nogood_sent_list) or (inconsistent-nogood-processor colorn) 
    [        
...
and
<li>Ionel Muscalagiu and V. Cretu. <a href="http://www.vtex.lt/Informatica/htm/INFO616.htm"> Improving the Performances of Asynchronous Algorithms by Combining the Nogood Processors with the Nogood Learning Techniques.</a> <i>Journal "INFORMATICA" Lithuania,  Vol. 17, Nr 1,  2006, pp (39-54). </i> 
</li>
</ul>
<p>
